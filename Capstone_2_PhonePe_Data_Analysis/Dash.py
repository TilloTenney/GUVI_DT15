...
icon = Image.open(r"C:\Users\vinti\PycharmProjects\pythonProject\Phonepe_Pulse_Capstone2\Phonepe.png")
st.set_page_config(page_title="PHONEPE PULSE DATA VISUALISATION | Tillo Tenney A E",
                   page_icon=icon,
                   layout="wide",
...
with st.sidebar:
    selected = option_menu(None, ["Home", "Analysis", "Insights"],
...
if selected == "Home":
    ...
    st.image(Image.open(r"C:\Users\vinti\PycharmProjects\pythonProject\Phonepe_Pulse_Capstone2\Phonepay.jpg"), width=600)
    col1,col2 = st.columns(2, gap='medium')
    col1.subheader(':violet[Phonepe Pulse]:')
    col1.write('PhonePe Pulse is a function provided by PhonePe, an Indian digital payments company. PhonePe Pulse provides customers with information and data relating to their digital transactions and PhonePe app usage patterns.')
    col1.subheader(':violet[Phonepe Pulse Data Visualisation]:')
    col1.write('The graphical depiction of data using charts, graphs, and other visual elements to help comprehension and analysis in a visually appealing manner is referred to as data visualization. The idea is to collect and process this data in order to obtain insights and information that can be visually shown in a user-friendly manner.')
    col2.video("https://youtu.be/yKvAS0p-qbQ")
    col2.markdown("#   ")
    col2.subheader(":violet[Technologies used]:")
    col2.write(
        """
        - Python and SQL.
    ...
    col2.markdown("#   ")
    col2.markdown("[ 👉 :orange[Download App]](https://www.phonepe.com/app-download/)")
    col2.markdown("[ 👉 :orange[Inspired from]](https://www.phonepe.com/pulse/)")
    col2.markdown("[ 👉 :orange[Githublink]](https://github.com/TilloTenney/Projects_TilloTenney.git)")
    col2.markdown("[ 👉 :orange[LinkedIn]](https://www.linkedin.com/in/tillo-tenney-a-e-aspiring-datascientist/)")
...
if selected == "Analysis":
    ...
    st.subheader('Between 2018 and 2023, an analysis was conducted on the basis of States wise, User wise and Top Categories from all over india.')
    ...
    if select == "INDIA":
        ...
        with tab1:
            ...
            mycursor1.execute(
                f"SELECT State, Transaction_amount FROM phonepe_pulse.aggregated_transaction WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
            in_tr_tab_qry_rslt = mycursor1.fetchall()
            df_in_tr_tab_qry_rslt = pd.DataFrame(np.array(in_tr_tab_qry_rslt), columns=['State', 'Transaction_amount'])
            print(df_in_tr_tab_qry_rslt)
            df_in_tr_tab_qry_rslt1 = df_in_tr_tab_qry_rslt.set_index(pd.Index(range(1, len(df_in_tr_tab_qry_rslt) + 1)))
            print(df_in_tr_tab_qry_rslt1)

            # Transaction Analysis table query
            mycursor1.execute(
                f"SELECT State, Transaction_count, Transaction_amount FROM phonepe_pulse.aggregated_transaction WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
            in_tr_anly_tab_qry_rslt = mycursor1.fetchall()
            ...
            mycursor1.execute(
                f"SELECT SUM(Transaction_amount), AVG(Transaction_amount) FROM phonepe_pulse.aggregated_transaction WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
            in_tr_am_qry_rslt = mycursor1.fetchall()
            ...
            mycursor1.execute(
                f"SELECT SUM(Transaction_count), AVG(Transaction_count) FROM phonepe_pulse.aggregated_transaction WHERE Year = '{in_tr_yr}' AND Quarter = '{in_tr_qtr}' AND Transaction_type = '{in_tr_tr_typ}';")
            in_tr_co_qry_rslt = mycursor1.fetchall()
        ...
        with tab2:
            ...
            mycursor1.execute(f"SELECT State, SUM(User_Count) FROM aggregated_user WHERE Year = '{in_us_yr}' AND Quarter = '{in_us_qtr}' GROUP BY State;")
            in_us_tab_qry_rslt = mycursor1.fetchall()
            ...
            mycursor1.execute(f"SELECT SUM(User_Count), AVG(User_Count) FROM aggregated_user WHERE Year = '{in_us_yr}' AND Quarter = '{in_us_qtr}';")
            in_us_co_qry_rslt = mycursor1.fetchall()
    ...
    if select == "STATES":
        ...
        with tab3:
            ...
            #Transaction Analysis bar chart query
            mycursor1.execute(f"SELECT Transaction_type, Transaction_amount FROM aggregated_transaction WHERE State = '{st_tr_st}' AND Year = '{st_tr_yr}' AND Quarter = '{st_tr_qtr}';")
            st_tr_tab_bar_qry_rslt = mycursor1.fetchall()
            ...
            # Transaction Analysis table query
            mycursor1.execute(f"SELECT Transaction_type, Transaction_count, Transaction_amount FROM aggregated_transaction WHERE State = '{st_tr_st}' AND Year = '{st_tr_yr}' AND Quarter = '{st_tr_qtr}';")
            st_tr_anly_tab_qry_rslt = mycursor1.fetchall()
            ...
            # Total Transaction Amount table query
            mycursor1.execute(f"SELECT SUM(Transaction_amount), AVG(Transaction_amount) FROM aggregated_transaction WHERE State = '{st_tr_st}' AND Year = '{st_tr_yr}' AND Quarter = '{st_tr_qtr}';")
            st_tr_am_qry_rslt = mycursor1.fetchall()
            ...
            # Total Transaction Count table query
            mycursor1.execute(f"SELECT SUM(Transaction_count), AVG(Transaction_count) FROM aggregated_transaction WHERE State = '{st_tr_st}' AND Year ='{st_tr_yr}' AND Quarter = '{st_tr_qtr}';")
            st_tr_co_qry_rslt = mycursor1.fetchall()
        ...
        with tab4:
            ...
            # User Analysis Bar chart query
            mycursor1.execute(f"SELECT Quarter, SUM(User_Count) FROM aggregated_user WHERE State = '{st_us_st}' AND Year = '{st_us_yr}' GROUP BY Quarter;")
            st_us_tab_qry_rslt = mycursor1.fetchall()
            ...
            # Total User Count table query
            mycursor1.execute(f"SELECT SUM(User_Count), AVG(User_Count) FROM aggregated_user WHERE State = '{st_us_st}' AND Year = '{st_us_yr}';")
            st_us_co_qry_rslt = mycursor1.fetchall()
    ...
    if select == "TOP CATEGORIES":
        ...
        with tab5:
            ...
            top_tr_tab_qry_rslt = mycursor1.fetchall()
            ...
            top_tr_anly_tab_qry_rslt = mycursor1.fetchall()
        ...
        with tab6:
            ...
            mycursor1.execute(f"SELECT State, SUM(Registered_User) AS Top_user FROM top_user WHERE Year='{top_us_yr}' GROUP BY State ORDER BY Top_user DESC LIMIT 10;")
            top_us_tab_qry_rslt = mycursor1.fetchall()
...
if selected == "Insights":
    ...
    st.subheader("The basic insights are derived from the Phonepe Pulse data analysis. It gives a clear picture of the analysed data.")
    options = ["--select--",
    ...
    #3
        ...
        mycursor1.execute("SELECT DISTINCT State, District_Pincode, SUM(Registered_User) AS Users FROM top_user GROUP BY State, District_Pincode ORDER BY Users DESC LIMIT 10");
        data = mycursor1.fetchall()
    ...
    #4
        ...
        mycursor1.execute("SELECT DISTINCT State, District_Pincode, SUM(Registered_User) AS Users FROM top_user GROUP BY State, District_Pincode ORDER BY Users ASC LIMIT 10");
        data = mycursor1.fetchall()
        columns = ['State', 'District_Pincode', 'Registered_User']
        df = pd.DataFrame(data, columns=columns, index=range(1, len(data) + 1))
        csv = df.to_csv().encode('utf-8')
    ...
    #10
        ...
        mycursor1.execute(
            "SELECT DISTINCT Mobile_Brands,SUM(User_Count) as Total FROM aggregated_user GROUP BY Mobile_Brands ORDER BY Total DESC LIMIT 10");
        data = mycursor1.fetchall()
        columns = ['Mobile_Brands', 'User_Count']
        df = pd.DataFrame(data, columns=columns, index=range(1, len(data) + 1))
        csv = df.to_csv().encode('utf-8')
        ...
        with col2:
            ...
            st.bar_chart(data=df , x="User_Count", y="Mobile_Brands")
